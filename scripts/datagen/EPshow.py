...
output_file = "../../data/EPshows_ref.json"
...
def extract_tvshow_data(input_path, output_path):
    ...
    for show in tv_shows:
        try:
            ...
            for season in show.get("seasons", []):
                ...
                for episode in season.get("episodes", []):
                    ...
                    episodes.append({
                        ...
                        "audioCodec": format_codec(episode_media.get("audioCodec")),
                        "container": episode_part.get("container"),
                        "videoCodec": format_codec(episode_media.get("videoCodec")),
                        "videoResolution": format_resolution(episode_media.get("videoResolution")),
                        "sizeHuman": episode_part.get("sizeHuman")
                    })
                ...
                season_data = {
                    ...
                    "avgSeasonVideoResolution": aggregate_unique([format_resolution(r) for r in season_video_resolutions]),
                    "avgSeasonAudioCodec": aggregate_unique([format_codec(c) for c in season_audio_codecs]),
                    "avgSeasonVideoCodec": aggregate_unique([format_codec(c) for c in season_video_codecs]),
            ...
            show_data = {
                ...
                "avgVideoResolutions": aggregate_unique([format_resolution(r) for r in show_video_resolutions]),
                "avgAudioCodecs": aggregate_unique([format_codec(c) for c in show_audio_codecs]),
                "avgVideoCodecs": aggregate_unique([format_codec(c) for c in show_video_codecs]),
    ...
    print(f"Cleaned up TV-Show[Episodes] data and saved to {output_path}")
